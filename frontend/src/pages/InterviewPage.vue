...
<template>
  <div class="grid gap-5 xl:grid-cols-[1fr_360px]">
    <Card>
      <CardHeader class="flex-row items-start justify-between">
        <div>
          ...
          <CardDescription>文字问答优先，AI 会根据回答生成追问。</CardDescription>
        </div>
        <Badge variant="accent">当前得分 {{ score || '--' }}</Badge>
      </CardHeader>
      <CardContent class="flex flex-col gap-5">
        ...
        <div class="flex flex-wrap gap-3">
          ...
          <Button variant="outline" :disabled="!interview || reportMutation.isPending.value" @click="reportMutation.mutate()">
            生成报告
          </Button>
        </div>
      </CardContent>
    ...
    <aside class="flex flex-col gap-5">
      <Card>
        <CardHeader>
          <CardTitle>评分摘要</CardTitle>
          <CardDescription>即时反馈会沉淀到报告。</CardDescription>
        </CardHeader>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>{{ streaming ? 'AI 正在生成...' : '根据上一轮回答动态生成。' }}</CardDescription>
        </CardHeader>
        <CardContent>
          <p class="text-sm leading-6 text-muted-foreground">{{ followUp || '暂无追问。' }}</p>
        </CardContent>
      </Card>
      <Card>
        <CardHeader>
        ...
        <CardContent class="flex flex-col gap-3">
          <div v-for="turn in interview?.turns" :key="turn.id" class="rounded-md border bg-background/70 p-3">
            <p class="text-sm font-medium">{{ turn.question }}</p>
            <p class="mt-1 text-xs text-muted-foreground">得分 {{ turn.score }}</p>
          </div>
          <p v-if="!interview?.turns.length" class="text-sm text-muted-foreground">还没有提交回答。</p>
        </CardContent>
      </Card>
    </aside>
  </div>
