...
- **简历 AI 诊断** — 上传简历后一键获取各模块评分、问题列表和改进建议
- **JD 关键词提取** — 从职位描述中自动提取结构化关键词，Dashboard 展示标签云
- **文档管理** — 支持文档删除（级联清理切片）、全文预览展开/收起
- **AI 助手 Markdown 渲染** — 助手消息支持标题、列表、代码块、表格等 Markdown 格式
- **面试计时器** — 每题倒计时（默认 5 分钟），剩余 60 秒变红提醒，面试结束统计总用时
- **全局进度引导条** — Dashboard 顶部展示全流程步骤（上传简历 → JD → 计划 → 题库 → 面试 → 报告），点击跳转
- **多维度趋势对比** — 报告页新增五线趋势图 + "进步最大/仍需改进"维度卡片
- **SSE 心跳机制** — 所有流式接口每 15 秒发送心跳，防止连接被代理断开
- **全局 Toast 通知** — API 错误自动弹出通知，支持成功/错误/警告/信息四种类型
- **JD 文本粘贴** — JD 上传支持"上传文件"和"粘贴文本"两种模式切换
- **STAR 话术卡片** — 一键为题库生成参考回答框架（STAR 结构 + 关键词 + 开头句）
- **面试真实模式** — 隐藏实时评分，模拟真实面试压力，面试结束后展示完整回顾
- **分享成绩单** — 报告页一键复制面试准备成绩（维度分数柱状图 + 统计数据）
- **简历优化重写** — 上传简历后一键获取 AI 改写建议（原文 vs 优化、缺失关键词、ATS 评分预估）
- **JD 匹配差距分析** — 自动对比简历与 JD 要求，标出已覆盖项和缺失项，给出弥补建议
- **四维评分详情展示** — 面试提交后实时展示各维度分数、优点列表和改进建议
- **简历诊断柱状图** — 诊断结果新增各模块柱状图可视化对比
- **PDF 报告导出** — 复盘报告一键导出为排版精美的 PDF 文件
- **答案历史对比** — 报告页展示所有面试的答案记录，跨面试对比得分变化
- **重新生成追问** — 追问区域新增"重新生成"按钮，不满意可重新获取追问
- **题库生成替换模式** — 每次生成题库自动替换旧题，避免题库无限膨胀
- **支持重复诊断** — 简历修改后可重新进行 AI 诊断，不再返回缓存

---

## 更新日志

### 2026-06-01 Phase 5：竞品级体验升级

**Bug 修复：**
- **修复面试得分算法**：从"新旧平均"改为"全轮次真实平均"，确保分数准确
- **修复 Roadmap 硬编码**：路线图的 milestones/focusAreas/strengths/gaps 由 LLM 根据简历和 JD 动态生成
- **修复分享卡片 fitScore=0**：分享成绩单的岗位匹配度从计划中正确读取

**体验升级：**
- **四维评分详情**：面试提交后实时展示各维度分数条、优点列表和改进建议
- **简历诊断柱状图**：诊断结果新增各模块柱状图可视化对比
- **题库生成替换模式**：每次生成题库自动替换旧题，避免题库无限膨胀
- **支持重复诊断**：简历修改后可重新进行 AI 诊断，按钮文案自动切换

**差异化功能：**
- **JD 匹配差距分析**：新增 `POST /api/prep-plans/jd-match` 接口，Dashboard 一键分析简历与 JD 的覆盖项和缺失项
- **简历优化重写**：新增 `POST /api/documents/{id}/rewrite` 接口，DocumentsPage 一键获取 AI 改写建议（原文 vs 优化、缺失关键词、ATS 评分预估）
- **PDF 报告导出**：ReportsPage 新增"导出 PDF"按钮，使用浏览器打印功能生成排版精美的 PDF
- **答案历史对比**：新增 `GET /api/interviews/history/answers` 接口，报告页展示所有面试的答案记录，跨面试对比得分变化
- **重新生成追问**：面试追问区域新增"重新生成"按钮
- **AI 助手增强**：助手上下文新增最近 5 轮面试对话，支持回答"我刚才第 X 题哪里答得不好"

### 2026-05-30 Phase 4：差异化功能

- **STAR 话术卡片**：新增 `POST /api/questions/answer-cards` 接口，为题库生成参考回答框架（STAR 提示 + 关键词 + 开头句）
- **面试真实模式**：面试页新增练习/真实模式切换，真实模式隐藏实时评分、支持计时器配置（3/5/8 分钟），面试结束后展示完整回顾
- **JD 文本粘贴**：新增 `POST /api/documents/job-description-text` 接口，DocumentsPage JD 区域支持上传文件/粘贴文本 Tab 切换
- **分享成绩单**：ReportsPage 新增 ShareCard 组件，一键复制面试准备成绩（柱状图 + 统计数据）
- **全局 Toast 通知**：新增 Pinia toast store + ToastContainer 组件，API 错误自动弹出通知，支持四种类型

### 2026-05-30 Phase 3：体验升级

- **AI 助手 Markdown 渲染**：助手消息支持 Markdown 格式（标题/列表/代码块/表格），使用 DOMPurify 防 XSS
- **面试计时器**：每题倒计时 5 分钟，不足 60 秒变红，提交后自动重启，面试结束统计总用时
- **全局进度引导条**：Dashboard 顶部展示全流程步骤，已完成步骤绿色勾选，当前步骤脉冲高亮
- **多维度趋势对比**：新增 `GET /api/reports/trend` 接口，五线趋势图 + "进步最大/仍需改进"维度卡片
- **SSE 心跳**：所有 SSE 流式接口每 15 秒发送 `:keepalive`，防止中间代理断开连接

### 2026-05-30 Phase 2：核心功能补齐

- **简历 AI 诊断评分**：新增 `POST /api/documents/{id}/analyze` 接口，LLM 分析简历各模块质量（教育/经历/项目/技能），输出评分 + 问题 + 建议
- **JD 关键词提取与可视化**：创建准备计划时自动提取 JD 关键词，Dashboard 展示彩色标签云（技术/软技能/经验/学历分类）
- **文档删除功能**：新增 `DELETE /api/documents/{id}` 接口，级联删除切片，前端带确认对话框
- **简历全文预览**：文档卡片支持展开/收起全文预览，显示上传时间
